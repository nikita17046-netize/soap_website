        ...
        <div className={styles.content}>
          {cart.length === 0 ? (
            ...
            <div className={styles.items}>
              {cart.map((item) => (
                <div key={item._id || item.id} className={`${styles.item} ${item.isOffer ? styles.offerItem : ''}`}>
                  ...
                  <div className={styles.itemInfo}>
                    ...
                    <div className={styles.quantity}>
                      <button onClick={() => updateQuantity(item._id || item.id, item.quantity - 1)} aria-label="Decrease quantity">−</button>
                      <span>{item.quantity}</span>
                      <button onClick={() => updateQuantity(item._id || item.id, item.quantity + 1)} aria-label="Increase quantity">+</button>
                    </div>
                  </div>
                  <button
